...
func TestShouldGetAllEvents(t *testing.T) {
	...
	assert.Equal(t, "Event 01", events[1].Name)
	assert.Equal(t, "Event 02", events[0].Name)
	assert.Equal(t, "Event 01 description", events[1].Description)
	assert.Equal(t, "Event 02 description", events[0].Description)
